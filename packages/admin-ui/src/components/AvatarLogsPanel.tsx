/**
 * Avatar Logs Panel - Consolidated log view for a single avatar.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { fetchAvatarLogs, type AvatarLogEvent } from '../api/logs';
import { fetchAvatarIssues, type AvatarIssue } from '../api/issues';
import { fetchAvatarEvents, type AvatarEvent, type AvatarFeedbackEvent, type AvatarIssueEvent } from '../api/events';
  ...
  if (jsonMatch) {
    try {
      ...
      if (typeof parsed === 'object' && parsed !== null) {
        const isIssue = parsed.event === 'avatar_reported_issue';
        return {
...
interface LogEntryProps {
  event: AvatarLogEvent;
  linkedIssue?: AvatarIssue;
...
export function AvatarLogsPanel({ avatarId, onMenuClick, onBack }: AvatarLogsPanelProps) {
  ...
  const issueByTimestamp = useMemo(() => {
    const map = new Map<string, AvatarIssue>();
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const response = await fetchAvatarEvents(avatarId, {
  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      const [logsResponse, issuesResponse] = await Promise.all([
        fetchAvatarLogs(avatarId, filters),
        fetchAvatarIssues(avatarId, { limit: 50 }),
      ]);
 ...
 */
function EventCard({ event }: { event: AvatarEvent }) {
  const [isExpanded, setIsExpanded] = useState(false);
